...
type htmlPayload struct {
	...
	TopRules     []htmlKV    `json:"topRules,omitempty"`
	TopResources []htmlKV    `json:"topResources,omitempty"`
}
...
func buildHTMLPayload(rep *Report) htmlPayload {
	...
	for _, f := range rep.Findings {
		ns := strings.TrimSpace(f.Subject.Namespace)
		if ns != "" {
			nsSet[ns] = true
		}
	}
	...
	// Top rules (by count desc, then key).
	for k, v := range rep.Summary.ByRule {
		if strings.TrimSpace(k) == "" || v <= 0 {
			continue
		}
		p.TopRules = append(p.TopRules, htmlKV{Key: k, Count: v})
	}
	sort.Slice(p.TopRules, func(i, j int) bool {
		if p.TopRules[i].Count != p.TopRules[j].Count {
			return p.TopRules[i].Count > p.TopRules[j].Count
		}
		return p.TopRules[i].Key < p.TopRules[j].Key
	})
	if len(p.TopRules) > 6 {
		p.TopRules = p.TopRules[:6]
	}

	// Top resources (by count desc, then key).
	resCounts := map[string]int{}
	for _, f := range rep.Findings {
		key := strings.TrimSpace(f.ResourceKey)
		if key == "" {
			key = strings.TrimSpace(resourceKey(f.Subject))
		}
		if key == "" {
			continue
		}
	...
	for k, v := range resCounts {
		if v <= 0 {
			continue
		}
		p.TopResources = append(p.TopResources, htmlKV{Key: k, Count: v})
	}
	sort.Slice(p.TopResources, func(i, j int) bool {
		if p.TopResources[i].Count != p.TopResources[j].Count {
			return p.TopResources[i].Count > p.TopResources[j].Count
		}
		return p.TopResources[i].Key < p.TopResources[j].Key
	})
	if len(p.TopResources) > 6 {
		p.TopResources = p.TopResources[:6]
	}
...
func writeHTML(w io.Writer, rep *Report) error {
	...
	b.WriteString("<title>verifier verify report</title>\n")
	b.WriteString("<style>\n")
	...
	b.WriteString(".hero{display:flex;gap:16px;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;} .hero>*{min-width:0;} \n")
	b.WriteString(".subtitle{font-size:0.98rem;color:var(--muted);overflow-wrap:anywhere;word-break:break-word;} \n")
	b.WriteString(".hero-right{max-width:560px;text-align:right;} \n")
	...
	b.WriteString("<script type=\"application/json\" id=\"verifierVerifyData\">")
...
const verifyHTMLJS = `
(function(){
  var raw = document.getElementById('verifierVerifyData');
  if(!raw){ return; }
  var data = {};
  ...
  function renderHero(){
    ...
    var left = ce('div');
    left.appendChild(text(ce('h1'), 'verifier verify report'));
