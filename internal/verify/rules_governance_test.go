...
func TestBuiltinRules_GovernanceAndSnapshots(t *testing.T) {
	...
	update := strings.TrimSpace(os.Getenv("VERIFIER_UPDATE_RULE_GOLDENS")) != ""
...
func writeOrCompareSnapshot(t *testing.T, update bool, path string, fs []Finding) {
	...
	if err != nil {
		t.Fatalf("read snapshot %s (set VERIFIER_UPDATE_RULE_GOLDENS=1 to generate): %v", path, err)
	}
	if string(want) != string(raw) {
		t.Fatalf("snapshot mismatch: %s (set VERIFIER_UPDATE_RULE_GOLDENS=1 to update)", path)
	}
}
