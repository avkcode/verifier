...
func TestWriteReport_HTML_Golden(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("read golden %s: %v (set VERIFIER_UPDATE_GOLDENS=1 to generate)", goldenPath, err)
	}
	if string(want) != got {
		t.Fatalf("golden mismatch (%s). set VERIFIER_UPDATE_GOLDENS=1 to update", goldenPath)
	}
}
