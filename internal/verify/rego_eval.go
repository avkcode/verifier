...
func EvaluateRulesWithSelectors(ctx context.Context, rules Ruleset, objects []map[string]any, commonDirs []string, selectors SelectorSet, ruleSelectors []RuleSelector) ([]Finding, error) {
	...
	for _, rule := range rules.Rules {
		...
		// - verifier.verify/ignore: "*" or comma/space-separated rule ids (e.g. "k8s/pss_restricted_profile")
...
func buildEvidenceWithExtras(obj map[string]any, fieldPath string, result map[string]any) map[string]any {
	...
	if v, ok := result["verifierChecksFailed"]; ok && v != nil {
		switch typed := v.(type) {
		case []string:
	...
	if v, ok := result["verifierContainerFailures"]; ok && v != nil {
...
func buildInputDocs(infos []objectInfo) ([]map[string]any, map[string]objectInfo) {
	...
	for i, info := range infos {
		...
		if len(info.annotations) > 0 {
			if raw := strings.TrimSpace(info.annotations["verifier.verify/ignore-checks"]); raw != "" {
				verifier["ignoreChecks"] = splitCSV(raw)
			}
		}
		doc := map[string]any{
			"id":       docID,
...
func shouldIgnoreRule(info objectInfo, ruleID string) bool {
	...
	raw := strings.TrimSpace(info.annotations["verifier.verify/ignore"])
	if raw == "" {
		return false
	}
