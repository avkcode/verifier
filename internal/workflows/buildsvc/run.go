...
import (
	...
	"verifier/internal/capture"
	"verifier/internal/caststream"
	"verifier/internal/castutil"
	"verifier/internal/dockerconfig"
	"verifier/internal/logging"
	"verifier/internal/tailer"
	"verifier/internal/telemetry"
	"verifier/internal/ui"
	"verifier/pkg/buildkit"
	appcompose "verifier/pkg/compose"
	"verifier/pkg/registry"
)
...
func (s *service) Run(ctx context.Context, opts Options) (res *Result, runErr error) {
	...
	if buildConsole != nil && cacheIntel != nil {
		deferredCacheIntel = writeCacheIntel
	}
	...
	if strings.TrimSpace(opts.AttestationDir) != "" {
		...
		if result.OCIOutputPath == "" {
			if stream != nil {
				stream.emitPhase("attest", "failed", "--attest-dir requires an OCI layout export but no OCI output path is available")
			}
		...
		if attErr != nil {
			if stream != nil {
				stream.emitPhase("attest", "failed", attErr.Error())
			}
			runErr = attErr
			return nil, attErr
		}
