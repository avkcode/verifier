...
func OpenMirrorStore(path string, opts MirrorStoreOptions) (MirrorStore, error) {
	...
	queueSize := envInt("VERIFIER_MIRROR_STORE_QUEUE_SIZE", 4096)
	if queueSize < 128 {
		queueSize = 128
	}
	batchSize := envInt("VERIFIER_MIRROR_STORE_BATCH_SIZE", 256)
	if batchSize < 16 {
		batchSize = 16
	}
	flushMS := envInt("VERIFIER_MIRROR_STORE_FLUSH_MS", 250)
	if flushMS < 25 {
		flushMS = 25
	}
...
INSERT OR IGNORE INTO verifier_mirror_sessions(
  session_id,
  created_at_ns,
  last_seen_ns,
  last_sequence,
...
`, sessionID, received, received, seq); err != nil {
		...
		if _, err := tx.ExecContext(ctx, `
UPDATE verifier_mirror_sessions
...
INSERT OR IGNORE INTO verifier_mirror_frames(session_id, sequence, received_ns, frame_proto)
VALUES(?, ?, ?, ?)
...
DELETE FROM verifier_mirror_frames
...
func (s *sqliteMirrorStore) DeleteSession(ctx context.Context, sessionID string) (bool, error) {
	...
	res, err := s.writeDB.ExecContext(ctx, `DELETE FROM verifier_mirror_sessions WHERE session_id = ?`, sessionID)
	if err != nil {
		return false, err
	}
...
func initMirrorSQLite(ctx context.Context, db *sql.DB) error {
	...
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS verifier_mirror_sessions (
  ...
  completed_at_ns INTEGER NOT NULL DEFAULT 0
		...
		`CREATE INDEX IF NOT EXISTS idx_mirror_sessions_last_seen ON verifier_mirror_sessions(last_seen_ns DESC);`,
		`CREATE TABLE IF NOT EXISTS verifier_mirror_frames (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  session_id TEXT NOT NULL,
  sequence INTEGER NOT NULL,
  received_ns INTEGER NOT NULL,
  frame_proto BLOB NOT NULL,
  FOREIGN KEY(session_id) REFERENCES verifier_mirror_sessions(session_id) ON DELETE CASCADE,
  UNIQUE(session_id, sequence)
);`,
		`CREATE INDEX IF NOT EXISTS idx_mirror_frames_session_seq ON verifier_mirror_frames(session_id, sequence);`,
		`CREATE INDEX IF NOT EXISTS idx_mirror_frames_session_received ON verifier_mirror_frames(session_id, received_ns);`,
	}
	...
	alter := []string{
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN command TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN args_json TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN requester TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN cluster TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN kube_context TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN namespace TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN release TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN chart TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN tags_json TEXT;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN state INTEGER NOT NULL DEFAULT 0;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN exit_code INTEGER NOT NULL DEFAULT 0;`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN error_message TEXT NOT NULL DEFAULT '';`,
		`ALTER TABLE verifier_mirror_sessions ADD COLUMN completed_at_ns INTEGER NOT NULL DEFAULT 0;`,
	}
...
func (s *sqliteMirrorStore) prune(ctx context.Context) error {
	...
	if s.maxSessions > 0 {
		var count int
		if err := tx.QueryRowContext(ctx, `SELECT COUNT(1) FROM verifier_mirror_sessions`).Scan(&count); err != nil {
			return err
		}
		excess := count - s.maxSessions
		if excess > 0 {
			if _, err := tx.ExecContext(ctx, `
DELETE FROM verifier_mirror_sessions
WHERE session_id IN (
  SELECT session_id FROM verifier_mirror_sessions ORDER BY last_seen_ns ASC LIMIT ?
)`, excess); err != nil {
				return err
			}
		}
...
DELETE FROM verifier_mirror_sessions
WHERE session_id IN (
  SELECT session_id FROM verifier_mirror_sessions ORDER BY last_seen_ns ASC LIMIT 10
)`)
			if err != nil {
				return err
			}
