...
// Package tailer implements verifier's high-performance, color-aware log streamer
// used by the 'verifier logs' family of commands, coordinating informers, filters,
// and observer hooks.
package tailer
...
func New(client kubernetes.Interface, opts *config.Options, logger logr.Logger, tailerOptions ...Option) (*Tailer, error) {
	...
	tmpl, err := template.New("verifier").Parse(opts.Template)
	if err != nil {
...
// DefaultColorPalette returns the vibrant color rotation used when rendering verifier log streams.
