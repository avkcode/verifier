...
func ExportRunBundle(ctx context.Context, root string, runID string, outPath string) (string, error) {
	...
	if strings.TrimSpace(outPath) == "" {
		outPath = filepath.Join(root, ".verifier", "stack", "exports", runID+".tgz")
	}
	...
	tmpRoot, err := os.MkdirTemp("", "verifier-stack-export-*")
	if err != nil {
		return "", err
	}
	...
	_ = dst.db.QueryRowContext(ctx, `SELECT run_digest FROM verifier_stack_runs WHERE run_id = ?`, runID).Scan(&runDigest)
	runDigest = strings.TrimSpace(runDigest)

	manifest := RunBundleManifest{
		APIVersion:  "verifier.dev/stack-bundle/v1",
		Kind:        "StackRunBundle",
...
FROM verifier_stack_runs WHERE run_id = ?
...
FROM verifier_stack_nodes WHERE run_id = ?
...
`, runID)
	...
	for rows.Next() {
		...
		_, err := dst.ExecContext(ctx, `
INSERT INTO verifier_stack_nodes (run_id, node_id, status, attempt, error, last_error_class, last_error_digest, updated_at_ns)
VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
`, runID)
	...
	for rows.Next() {
		...
		_, err := dst.ExecContext(ctx, `
INSERT INTO verifier_stack_events (run_id, ts_ns, node_id, type, attempt, message, error_class, error_message, error_digest, seq, prev_digest, digest, crc32)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func ExtractBundleToTempDir(bundlePath string) (string, error) {
	// Uses `tar` for simplicity and speed; bundle format is controlled by verifier.
	...
	tmp, err := os.MkdirTemp("", "verifier-bundle-*")
	if err != nil {
		return "", err
	}
