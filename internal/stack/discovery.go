...
func readStackFile(path string) (*StackFile, error) {
	...
	if sf.APIVersion != "" && sf.APIVersion != "verifier.dev/v1" {
		return nil, fmt.Errorf("%s: apiVersion must be verifier.dev/v1 (got %q)", path, sf.APIVersion)
	}
	for i := range sf.Releases {
...
func readReleaseFile(path string) (*ReleaseFile, error) {
	...
	if rf.APIVersion != "" && rf.APIVersion != "verifier.dev/v1" {
		return nil, fmt.Errorf("%s: apiVersion must be verifier.dev/v1 (got %q)", path, rf.APIVersion)
	}
