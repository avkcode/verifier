...
func TestDisasterRecovery_KillAndReplayDoesNotCorruptSQLite(t *testing.T) {
	if os.Getenv("VERIFIER_STACK_TEST_HELPER") == "1" {
		helperMain(t)
		return
	}
	...
	cmd.Env = append(os.Environ(),
		"VERIFIER_STACK_TEST_HELPER=1",
		"VERIFIER_STACK_TEST_ROOT="+root,
	)
...
func helperMain(t *testing.T) {
	root := os.Getenv("VERIFIER_STACK_TEST_ROOT")
	if strings.TrimSpace(root) == "" {
		t.Fatal("missing VERIFIER_STACK_TEST_ROOT")
	}
...
`
	...
	stackYAML := fmt.Sprintf(`apiVersion: verifier.dev/v1
kind: Stack
name: %s
defaults:
  cluster: { name: c1 }
