...
func TestListRuns_NewestFirst(t *testing.T) {
	...
	if _, err := s.db.ExecContext(ctx, `UPDATE verifier_stack_runs SET created_at_ns = ?, updated_at_ns = ?, summary_json = ? WHERE run_id = ?`,
...
func TestRunStatus_FormatTable(t *testing.T) {
	...
	if _, err := s.db.ExecContext(ctx, `UPDATE verifier_stack_runs SET summary_json = ? WHERE run_id = ?`, marshalJSON(t, sum), runID); err != nil {
