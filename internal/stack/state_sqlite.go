...
const stackStateSQLiteRelPath = ".verifier/stack/state.sqlite"
...
FROM verifier_stack_node_steps
WHERE run_id = ?
...
func (s *stackStateStore) initSchema(ctx context.Context) error {
	stmts := []string{
		...
		`
CREATE TABLE IF NOT EXISTS verifier_stack_runs (
  ...
  run_digest TEXT NOT NULL DEFAULT ''
		...
		`
	CREATE TABLE IF NOT EXISTS verifier_stack_nodes (
  ...
  FOREIGN KEY (run_id) REFERENCES verifier_stack_runs(run_id) ON DELETE CASCADE
	);`,
		`
	CREATE TABLE IF NOT EXISTS verifier_stack_node_steps (
	  run_id TEXT NOT NULL,
	  node_id TEXT NOT NULL,
	  attempt INTEGER NOT NULL,
	  step TEXT NOT NULL,
	  ...
	  FOREIGN KEY (run_id) REFERENCES verifier_stack_runs(run_id) ON DELETE CASCADE
		...
		`
	CREATE TABLE IF NOT EXISTS verifier_stack_events (
  ...
  FOREIGN KEY (run_id) REFERENCES verifier_stack_runs(run_id) ON DELETE CASCADE
		...
		`CREATE INDEX IF NOT EXISTS idx_verifier_stack_events_run_id_id ON verifier_stack_events(run_id, id);`,
		`CREATE INDEX IF NOT EXISTS idx_verifier_stack_events_run_id_error_digest ON verifier_stack_events(run_id, error_digest);`,
		`CREATE INDEX IF NOT EXISTS idx_verifier_stack_nodes_run_id_status ON verifier_stack_nodes(run_id, status);`,
		`CREATE INDEX IF NOT EXISTS idx_verifier_stack_node_steps_by_run_node ON verifier_stack_node_steps(run_id, node_id);`,
		`
	CREATE TABLE IF NOT EXISTS verifier_stack_lock (
  ...
  ttl_ns INTEGER NOT NULL
		...
		`
CREATE TABLE IF NOT EXISTS verifier_stack_apply_cache (
  ...
  PRIMARY KEY (cluster_key, namespace, release_name, command, effective_input_hash)
		...
		`CREATE INDEX IF NOT EXISTS idx_verifier_stack_apply_cache_lookup ON verifier_stack_apply_cache(cluster_key, namespace, release_name, command, effective_input_hash);`,
		`
CREATE TABLE IF NOT EXISTS verifier_stack_verify_cache (
  cluster_key TEXT NOT NULL,
  namespace TEXT NOT NULL,
  release_name TEXT NOT NULL,
  ...
  PRIMARY KEY (cluster_key, namespace, release_name)
		...
		`CREATE INDEX IF NOT EXISTS idx_verifier_stack_verify_cache_lookup ON verifier_stack_verify_cache(cluster_key, namespace, release_name);`,
	}
...
func (s *stackStateStore) ensureVerifyCacheColumns(ctx context.Context) error {
	cols, err := s.tableColumns(ctx, "verifier_stack_verify_cache")
	if err != nil {
		return err
	}
	...
	for name, ddl := range want {
		...
		if _, err := s.db.ExecContext(ctx, fmt.Sprintf("ALTER TABLE verifier_stack_verify_cache ADD COLUMN %s %s;", name, ddl)); err != nil {
			return fmt.Errorf("add column verifier_stack_verify_cache.%s: %w", name, err)
		}
	}
	return nil
}
...
FROM verifier_stack_apply_cache
...
INSERT INTO verifier_stack_apply_cache (
...
ON CONFLICT(cluster_key, namespace, release_name, command, effective_input_hash) DO UPDATE SET
  ...
  updated_at_ns = CASE WHEN excluded.updated_at_ns > verifier_stack_apply_cache.updated_at_ns THEN excluded.updated_at_ns ELSE verifier_stack_apply_cache.updated_at_ns END
...
FROM verifier_stack_verify_cache
...
func (s *stackStateStore) UpsertVerifyCache(ctx context.Context, key VerifyCacheKey, checkedAtNS int64, okAtNS int64, result string, message string, lastEventRVJSON string, evidenceJSON string) error {
	...
	_, err := s.db.ExecContext(ctx, `
INSERT INTO verifier_stack_verify_cache (
...
ON CONFLICT(cluster_key, namespace, release_name) DO UPDATE SET
  last_ok_at_ns = CASE WHEN excluded.last_ok_at_ns > 0 THEN excluded.last_ok_at_ns ELSE verifier_stack_verify_cache.last_ok_at_ns END,
  last_checked_at_ns = CASE WHEN excluded.last_checked_at_ns > 0 THEN excluded.last_checked_at_ns ELSE verifier_stack_verify_cache.last_checked_at_ns END,
  last_result = CASE WHEN excluded.last_result != '' THEN excluded.last_result ELSE verifier_stack_verify_cache.last_result END,
  last_message = CASE WHEN excluded.last_message != '' THEN excluded.last_message ELSE verifier_stack_verify_cache.last_message END,
  last_event_rv_json = CASE WHEN excluded.last_event_rv_json != '' THEN excluded.last_event_rv_json ELSE verifier_stack_verify_cache.last_event_rv_json END,
  last_evidence_json = CASE WHEN excluded.last_evidence_json != '' THEN excluded.last_evidence_json ELSE verifier_stack_verify_cache.last_evidence_json END,
  updated_at_ns = CASE WHEN excluded.updated_at_ns > verifier_stack_verify_cache.updated_at_ns THEN excluded.updated_at_ns ELSE verifier_stack_verify_cache.updated_at_ns END
...
func (s *stackStateStore) ensureEventsIntegrityColumns(ctx context.Context) error {
	cols, err := s.tableColumns(ctx, "verifier_stack_events")
	if err != nil {
		return err
	}
	want := map[string]string{
	...
	for name, ddl := range want {
		...
		if _, err := s.db.ExecContext(ctx, fmt.Sprintf("ALTER TABLE verifier_stack_events ADD COLUMN %s %s;", name, ddl)); err != nil {
			return fmt.Errorf("add column verifier_stack_events.%s: %w", name, err)
		}
	}
	return nil
}
...
func (s *stackStateStore) ensureRunColumns(ctx context.Context) error {
	cols, err := s.tableColumns(ctx, "verifier_stack_runs")
	if err != nil {
		return err
	}
	want := map[string]string{
	...
	for name, ddl := range want {
		...
		if _, err := s.db.ExecContext(ctx, fmt.Sprintf("ALTER TABLE verifier_stack_runs ADD COLUMN %s %s;", name, ddl)); err != nil {
			return fmt.Errorf("add column verifier_stack_runs.%s: %w", name, err)
		}
	}
	return nil
}
...
func (s *stackStateStore) ensureNodeColumns(ctx context.Context) error {
	cols, err := s.tableColumns(ctx, "verifier_stack_nodes")
	if err != nil {
		return err
	}
	want := map[string]string{
	...
	for name, ddl := range want {
		...
		if _, err := s.db.ExecContext(ctx, fmt.Sprintf("ALTER TABLE verifier_stack_nodes ADD COLUMN %s %s;", name, ddl)); err != nil {
			return fmt.Errorf("add column verifier_stack_nodes.%s: %w", name, err)
		}
	}
	return nil
}
...
func (s *stackStateStore) CreateRun(ctx context.Context, run *runState, p *Plan) error {
	...
	payload.VerifierVersion = version.Version
	payload.VerifierGitCommit = version.GitCommit
	...
	emptySummary := RunSummary{
		APIVersion: "verifier.dev/stack-run/v1",
		RunID:      run.RunID,
		Status:     "created",
...
INSERT INTO verifier_stack_runs (
...
`, run.RunID, p.StackRoot, p.StackName, p.Profile, run.Command, run.Concurrency, run.FailMode, "running",
	...
	for _, n := range run.Nodes {
		_, err := tx.ExecContext(ctx, `
INSERT INTO verifier_stack_nodes (run_id, node_id, status, attempt, error)
VALUES (?, ?, ?, ?, ?)
...
func (s *stackStateStore) AppendEvent(ctx context.Context, runID string, ev RunEvent) error {
	...
	_, err = s.db.ExecContext(ctx, `
	INSERT INTO verifier_stack_events (run_id, ts_ns, node_id, type, attempt, message, fields_json, error_class, error_message, error_digest, seq, prev_digest, digest, crc32)
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	...
	_, _ = s.db.ExecContext(ctx, `UPDATE verifier_stack_runs SET updated_at_ns = ? WHERE run_id = ?`, updatedAt, runID)
...
`, status, ev.Attempt, ev.Attempt, nodeErr, lastErrClass, lastErrDigest, updatedNodeAt, runID, nodeID)
	...
	if ev.Type == "RUN_COMPLETED" {
		...
		_, _ = s.db.ExecContext(ctx, `UPDATE verifier_stack_runs SET status = ?, updated_at_ns = ? WHERE run_id = ?`, status, updatedAt, runID)
	}
	return nil
}
...
func (s *stackStateStore) WriteSummary(ctx context.Context, runID string, summary *RunSummary) error {
	...
	_, err = tx.ExecContext(ctx, `UPDATE verifier_stack_runs SET summary_json = ?, status = ?, updated_at_ns = ? WHERE run_id = ?`, string(raw), summary.Status, updatedAt, runID)
	if err != nil {
		return err
	}
...
func (s *stackStateStore) FinalizeRun(ctx context.Context, runID string, completedAtNS int64, lastEventDigest string) (string, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT plan_json, summary_json FROM verifier_stack_runs WHERE run_id = ?`, runID).Scan(&planJSON, &summaryJSON); err != nil {
		return "", err
	}
	digest := computeRunDigest(planJSON, summaryJSON, lastEventDigest)
	_, err := s.db.ExecContext(ctx, `
UPDATE verifier_stack_runs
...
func (s *stackStateStore) GetRunSummary(ctx context.Context, runID string) (*RunSummary, error) {
	var raw string
	err := s.db.QueryRowContext(ctx, `SELECT summary_json FROM verifier_stack_runs WHERE run_id = ?`, runID).Scan(&raw)
	if err != nil {
		return nil, err
	}
	var ssum RunSummary
...
func (s *stackStateStore) GetRunPlan(ctx context.Context, runID string) (*Plan, error) {
	var raw string
	err := s.db.QueryRowContext(ctx, `SELECT plan_json FROM verifier_stack_runs WHERE run_id = ?`, runID).Scan(&raw)
	if err != nil {
		return nil, err
	}
	var rp RunPlan
...
func (s *stackStateStore) GetNodeStatus(ctx context.Context, runID string) (map[string]string, map[string]int, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT node_id, status, attempt FROM verifier_stack_nodes WHERE run_id = ?`, runID)
	if err != nil {
		return nil, nil, err
	}
	defer rows.Close()
...
func (s *stackStateStore) upsertNodeStepFromEvent(ctx context.Context, runID string, tsNS int64, nodeID string, ev RunEvent, errClass, errMsg, errDigest string) error {
	...
	_, err := s.db.ExecContext(ctx, `
INSERT INTO verifier_stack_node_steps (
  run_id, node_id, attempt, step,
  started_at_ns, completed_at_ns, status, message,
  error_class, error_message, error_digest, cursor_json
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT(run_id, node_id, attempt, step) DO UPDATE SET
  started_at_ns = CASE WHEN excluded.started_at_ns > 0 THEN excluded.started_at_ns ELSE verifier_stack_node_steps.started_at_ns END,
  completed_at_ns = CASE WHEN excluded.completed_at_ns > 0 THEN excluded.completed_at_ns ELSE verifier_stack_node_steps.completed_at_ns END,
  status = CASE WHEN excluded.status != '' THEN excluded.status ELSE verifier_stack_node_steps.status END,
  message = CASE WHEN excluded.message != '' THEN excluded.message ELSE verifier_stack_node_steps.message END,
  error_class = CASE WHEN excluded.error_class != '' THEN excluded.error_class ELSE verifier_stack_node_steps.error_class END,
  error_message = CASE WHEN excluded.error_message != '' THEN excluded.error_message ELSE verifier_stack_node_steps.error_message END,
  error_digest = CASE WHEN excluded.error_digest != '' THEN excluded.error_digest ELSE verifier_stack_node_steps.error_digest END,
  cursor_json = CASE WHEN excluded.cursor_json != '' THEN excluded.cursor_json ELSE verifier_stack_node_steps.cursor_json END
...
func (s *stackStateStore) MostRecentRunID(ctx context.Context) (string, error) {
	var runID string
	err := s.db.QueryRowContext(ctx, `SELECT run_id FROM verifier_stack_runs ORDER BY created_at_ns DESC LIMIT 1`).Scan(&runID)
	return runID, err
}
...
func (s *stackStateStore) GetLock(ctx context.Context) (*StackLock, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT owner, run_id, created_at_ns, ttl_ns FROM verifier_stack_lock WHERE id = 1`).Scan(&owner, &runID, &createdAtNS, &ttlNS)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
func (s *stackStateStore) AcquireLock(ctx context.Context, owner string, ttl time.Duration, takeover bool, runID string) (*StackLock, error) {
	...
	var createdAtNS, ttlNS int64
	err = tx.QueryRowContext(ctx, `SELECT owner, run_id, created_at_ns, ttl_ns FROM verifier_stack_lock WHERE id = 1`).Scan(&curOwner, &curRunID, &createdAtNS, &ttlNS)
	...
	if err == nil {
		...
		_, err := tx.ExecContext(ctx, `UPDATE verifier_stack_lock SET owner = ?, run_id = ?, created_at_ns = ?, ttl_ns = ? WHERE id = 1`,
			owner, strings.TrimSpace(runID), now.UnixNano(), ttl.Nanoseconds())
		if err != nil {
			return nil, err
		}
	} else {
		_, err := tx.ExecContext(ctx, `INSERT INTO verifier_stack_lock (id, owner, run_id, created_at_ns, ttl_ns) VALUES (1, ?, ?, ?, ?)`,
			owner, strings.TrimSpace(runID), now.UnixNano(), ttl.Nanoseconds())
		if err != nil {
			return nil, err
		}
...
func (s *stackStateStore) ReleaseLock(ctx context.Context, owner string, runID string) error {
	...
	_, _ = s.db.ExecContext(ctx, `DELETE FROM verifier_stack_lock WHERE id = 1 AND owner = ? AND run_id = ?`, owner, runID)
	return nil
}
