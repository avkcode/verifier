...
`, runID).Scan(&status, &createdAtNS, &updatedAtNS, &completedAtNS, &createdBy, &host, &pid, &ciRunURL, &gitAuthor, &kubeconfig, &kubeContext, &planJSON, &summaryJSON, &storedLastHash, &storedRunHash)
	...
	a := &RunAudit{
		APIVersion:    "verifier.dev/stack-audit/v1",
		RunID:         runID,
	...
	row := s.db.QueryRowContext(ctx, `SELECT digest FROM verifier_stack_events WHERE run_id = ? ORDER BY id DESC LIMIT 1`, runID)
...
func buildStackStatusFollowCommand(root string, runID string) string {
	...
	if root == "." {
		return "verifier stack status --run-id " + runID + " --follow"
	}
	return "verifier stack --root " + root + " status --run-id " + runID + " --follow"
}
