...
func TestStackStateSQLite_CheckpointPortable_SingleFileCopyOpens(t *testing.T) {
	...
	planJSON := `{"apiVersion":"verifier.dev/stack-plan/v1"}`
	summaryJSON := `{"apiVersion":"verifier.dev/stack-run/v1"}`
...
`, runID, "node-1", "planned", 0, "")
	...
	_, err = s.db.ExecContext(ctx, `
INSERT INTO verifier_stack_events (run_id, ts_ns, node_id, type, attempt, message, error_class, error_message, error_digest)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
`, runID, now, "node-1", "NODE_RUNNING", 1, "", "", "", "")
	...
	var runs int
	if err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM verifier_stack_runs;`).Scan(&runs); err != nil {
	...
	var events int
	if err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM verifier_stack_events;`).Scan(&events); err != nil {
