...
func providerHints(defaultProvider string, providers []string) []string {
	...
	if strings.TrimSpace(defaultProvider) == "" {
		hints = append(hints, "set secrets.defaultProvider in .verifier.yaml or pass --secret-provider")
	}
	return hints
}
...
func validateVaultRef(ctx context.Context, p *vaultProvider, ref Ref, opts ValidationOptions) *ValidationIssue {
	...
	if err != nil {
		...
		if parent, _ := parentPath(path); parent != "" {
			hints = append(hints, fmt.Sprintf("list vault path: verifier secrets list --secret-provider %s --path %s", ref.Provider, parent))
		} else {
			hints = append(hints, fmt.Sprintf("list vault path: verifier secrets list --secret-provider %s --path /", ref.Provider))
		}
		return &ValidationIssue{
			Reference:   ref.Reference(),
			Provider:    ref.Provider,
