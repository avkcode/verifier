...
func TestRecorderWritesSessionAndEvents(t *testing.T) {
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM verifier_capture_sessions`).Scan(&sessionCount); err != nil {
	...
	var eventCount int
	if err := db.QueryRow(`SELECT COUNT(*) FROM verifier_capture_events`).Scan(&eventCount); err != nil {
	...
	if err := db.QueryRow(`SELECT MIN(seq), MAX(seq) FROM verifier_capture_events`).Scan(&minSeq, &maxSeq); err != nil {
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM verifier_capture_artifacts`).Scan(&artifactCount); err != nil {
