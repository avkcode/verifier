...
func DefaultBuilderAddress() string {
	if v := os.Getenv("VERIFIER_BUILDKIT_HOST"); v != "" {
		return v
	}
...
func DefaultCacheDir() string {
	if v := os.Getenv("VERIFIER_BUILDKIT_CACHE"); v != "" {
		return v
	}
	if cacheDir, err := os.UserCacheDir(); err == nil {
		return filepath.Join(cacheDir, "verifier", "buildkit-cache")
	}
	if home, err := os.UserHomeDir(); err == nil && home != "" {
		return filepath.Join(home, ".cache", "verifier", "buildkit-cache")
	}
	return filepath.Join(os.TempDir(), "verifier-buildkit-cache")
}
...
func DefaultLocalTag(contextDir string) string {
	...
	return fmt.Sprintf("verifier.local/%s:dev", sanitized)
}
